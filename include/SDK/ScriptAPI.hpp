...
namespace Universe::Scripting::API
{
    ...
    using PropertyCallback       = void(IPropertyCallbackInfo& info);

    // Constructor provides internal data for the object on creation
    using ConstructorCallback = void(IObject& object);
    ...
    public:
        ...
        virtual void* GetInternal(const std::string& k) = 0;
    ...
    public:
        virtual void SetFunction(const std::string& name, FunctionCallback callback) = 0;
        virtual void SetProperty(const std::string& name, PropertyCallback callback) = 0;

        virtual IObject& CreateInstance(std::unordered_map<std::string, void*> internalData) = 0;
    };

    class IClassRegistry {
    public:
        virtual IClassDefinition* RegisterClass(const std::string& name) = 0;
        virtual IClassDefinition* GetClass     (const std::string& name) = 0;
    };
