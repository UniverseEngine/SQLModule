    ...
    : wxDialog(NULL, wxID_ANY, title, wxDefaultPosition)
{
    ...
    /* General */
    {
        auto panel = std::make_unique<wxPanel>(m_notebook);
        m_notebook->AddPage(panel.get(), "General", true);

        auto sizer = std::make_unique<wxBoxSizer>(wxVERTICAL);

        /* nickname */
        auto nickname_sizer = std::make_unique<wxStaticBoxSizer>(wxVERTICAL, panel, "Nickname");
        {
            m_nicknameInput = std::make_unique<wxTextCtrl>(panel, wxID_ANY, "", wxDefaultPosition, wxSize(240, 24));
            m_nicknameInput->SetValue(gBrowser->m_settings.nickname);

            nickname_sizer->Add(m_nicknameInput.get(), 1, wxALL | wxEXPAND, 5);
        }

        sizer->Add(nickname_sizer.get(), 0, wxALL | wxEXPAND, 5);

        /* masterlist */
        auto masterlist_sizer = std::make_unique<wxStaticBoxSizer>(wxVERTICAL, panel, "Masterlist");
        {
            m_masterlistInput = std::make_unique<wxTextCtrl>(panel, wxID_ANY, "", wxDefaultPosition, wxSize(240, 24));
            m_masterlistInput->SetValue(gBrowser->m_settings.masterlist);

            masterlist_sizer->Add(m_masterlistInput.get(), 1, wxALL | wxEXPAND, 5);
        }

        sizer->Add(masterlist_sizer.get(), 0, wxALL | wxEXPAND, 5);
    ...
    /* GTA:III Settings */
    {
        ...
        /* game path */
        auto gamePath_sizer = std::make_unique<wxStaticBoxSizer>(wxVERTICAL, panel.get(), "Install std::filesystem::path");
        {
            m_gamePath = std::make_unique<wxTextCtrl>(panel, wxID_ANY, "", wxDefaultPosition, wxSize(280, 24));
            m_gamePath->SetValue(gBrowser->m_settings.gamePath.string());

            auto browseButton = std::make_unique<wxButton>(panel, DIALOG_BROWSE, "Browse", wxDefaultPosition, wxSize(280, 24));
            browseButton->Bind(wxEVT_BUTTON, &SettingsDialog::OnBrowse, this);

            gamePath_sizer->Add(m_gamePath.get(), 1, wxALL | wxEXPAND, 5);
            gamePath_sizer->Add(browseButton.get(), 1, wxALL | wxEXPAND, 5);
        }

        sizer->Add(gamePath_sizer.get(), 0, wxALL | wxEXPAND, 5);

        /* settings */
        auto gameSettings_sizer = std::make_unique<wxStaticBoxSizer>(wxVERTICAL, panel, "Game Settings");
        {
            m_windowedCheckbox = std::make_unique<wxCheckBox>(panel, DIALOG_WINDOWED, "Windowed");
            m_windowedCheckbox->SetValue(gBrowser->m_settings.windowed);
            gameSettings_sizer->Add(m_windowedCheckbox.get(), 1);

            gameSettings_sizer->SetMinSize(150, 0);
        }

        sizer->Add(gameSettings_sizer.get(), 0, wxALL | wxEXPAND, 5);
    ...
    /* Developer Settings */
    {
        ...
        /* developer settings */
        auto developerSettings_sizer = std::make_unique<wxStaticBoxSizer>(wxVERTICAL, panel, "Developer Settings");
        {
            /* checkbox: Show console */
            m_showConsoleCheckbox = std::make_unique<wxCheckBox>(panel, DIALOG_WINDOWED, "Show console");
            m_showConsoleCheckbox->SetValue(gBrowser->m_settings.showConsole);
            developerSettings_sizer->Add(m_showConsoleCheckbox.get(), 1);

            developerSettings_sizer->SetMinSize(150, 0);
        }

        sizer->Add(developerSettings_sizer.get(), 0, wxALL | wxEXPAND, 5);
